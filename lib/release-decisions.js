...
function participantRoles(escrow) {
  ...
  // We fall back to creator+counterparty if funder rows aren't loaded yet,
  // but escrow.js will have attached escrow._funders from listFunders().
  if (model === 'n_of_m') {
    const funderPubkeys = Array.isArray(escrow._funders)
      ? escrow._funders.map(f => f.funder_pubkey).filter(Boolean)
      : [];
    const all = funderPubkeys.length > 0
      ? funderPubkeys
      : [creator, counterparty].filter(Boolean);
    return { creator, counterparty, all, funders: all };
  }
