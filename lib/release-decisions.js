 ...
 * n_of_m:        all escrow_funders rows are participants. The creator is
...
function participantRoles(escrow) {
  ...
  // n_of_m: participants are all pubkeys in escrow_funders.
  ...
  if (model === 'n_of_m') {
    if (!Array.isArray(escrow._funders) || escrow._funders.length === 0) {
      throw new Error('n_of_m escrow requires loaded funder rows (escrow._funders) for role resolution');
    }
...
function verifyDecision(escrow, action, body) {
  ...
  if (action === 'refund' && recipient !== 'creator') {
    ...
    // In two_party / n_of_m, refunds also go to creator (the lead funder);
