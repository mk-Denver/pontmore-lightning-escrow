 ...
 *
 * single_funder: creator (funder) + counterparty (recipient-on-release).
 * two_party:     creator + counterparty — both are funders and participants.
 * m_of_n:        all escrow_funders rows are participants. The creator is
 *                always a participant; additional participants joined via invitation.
 */
function participantRoles(escrow) {
  ...
  if (model === 'single_funder') {
    return {
      creator,
      counterparty,
      all: [creator, counterparty].filter(Boolean),
      funders: [creator].filter(Boolean),
    };
  }
  ...
  // m_of_n: participants are all pubkeys in escrow_funders.
  // escrow.js attaches escrow._funders from listFunders() before calling.
  // Require a populated funder-row array; reject if unavailable.
  if (model === 'm_of_n') {
    if (!Array.isArray(escrow._funders) || escrow._funders.length === 0) {
      throw new Error('m_of_n escrow requires loaded funder rows (escrow._funders) for role resolution');
    }
    const funderPubkeys = escrow._funders.map(f => f.funder_pubkey).filter(Boolean);
    return { creator, counterparty, all: funderPubkeys, funders: funderPubkeys };
  }
...
function verifyDecision(escrow, action, body) {
  ...
  const recipientIsRole = recipient === 'creator' || recipient === 'counterparty';
  const recipientIsBoundPubkey = escrow.funding_model === 'm_of_n'
    && isHexPubkey(recipient)
    && roles.all.includes(recipient);
  if (!recipientIsRole && !recipientIsBoundPubkey) {
    return { ok: false, error: '"recipient" must identify a bound participant' };
  }

  if (action === 'refund' && recipient !== 'creator') {
    // In single_funder, refunds return to the funder (creator).
    // In two_party / m_of_n, refunds also go to creator (the lead funder);
    // the service distributes proportional refunds to all funders internally.
  ...
  if (decisionType === 'threshold_participant_signatures') {
    const threshold = escrow.funding_model === 'm_of_n' ? escrow.funding_threshold : roles.all.length;
    let validCount = 0;
    const seen = new Set();
