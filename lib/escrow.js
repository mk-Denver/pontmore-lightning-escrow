...
async function create({ authPubkey, amountSats, description, refundLnAddress, idempotencyKey, invitationToken, fundingModel, fundingThreshold, participantCount }) {
  ...
  if (model === 'two_party') {
    ...
    // n_of_m is always client-defined: M and N must be supplied on each create.
    if (!Number.isInteger(fundingThreshold) || fundingThreshold < 1) {
      throw new ValidationError('n_of_m requires a positive integer funding_threshold (M) in the request');
    }
    if (!Number.isInteger(participantCount) || participantCount < 1) {
      throw new ValidationError('n_of_m requires a positive integer participant_count (N) in the request');
    }
