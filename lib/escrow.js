...
async function create({ authPubkey, amountSats, description, refundLnAddress, idempotencyKey, invitationToken, fundingModel, fundingThreshold, participantCount }) {
  ...
  if (model === 'two_party') {
    ...
    resolvedThreshold = Number.isInteger(fundingThreshold) && fundingThreshold > 0
      ? fundingThreshold
      : config.FUNDING_THRESHOLD;
    resolvedParticipantCount = Number.isInteger(participantCount) && participantCount > 0
      ? participantCount
      : config.PARTICIPANT_COUNT;
    if (!resolvedThreshold || !resolvedParticipantCount) {
      throw new ValidationError('n_of_m requires funding_threshold (M) and participant_count (N) — either in the request or as deployment defaults');
    }
    if (resolvedThreshold > resolvedParticipantCount) {
...
async function fundStatus({ escrowId, authPubkey }) {
  ...
  if (model === 'two_party') {
    ...
    if (!Number.isInteger(escrow.funding_threshold) || escrow.funding_threshold < 1) {
      throw new ValidationError(`n_of_m escrow ${escrow.escrow_id} is missing a valid funding_threshold`);
    }
...
async function refund({ escrowId, authPubkey, decisionBody }) {
  ...
  // two_party / n_of_m: refund each funded funder their contributed amount.
