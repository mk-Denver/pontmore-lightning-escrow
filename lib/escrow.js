...
async function create({ authPubkey, amountSats, description, refundLnAddress, idempotencyKey, enrollmentToken, participantPubkeys, fundingModel, fundingThreshold, participantCount }) {
  ...
  // Per-instance funding model: use the request value if provided and accepted,
  // otherwise fall back to the deployment default.
  const model = fundingModel || config.FUNDING_MODEL;
  ...
  if (model === 'two_party') {
    ...
    if (!Number.isInteger(participantCount) || participantCount < 1) {
      throw new ValidationError('m_of_n requires a positive integer participant_count (N) in the request');
    }
  ...
  if (!Array.isArray(participantPubkeys) || participantPubkeys.length !== expectedInvitees) {
    throw new ValidationError(`${model} requires exactly ${expectedInvitees} participant_pubkeys`);
  }
  if (new Set(participantPubkeys).size !== participantPubkeys.length || participantPubkeys.includes(authPubkey) || participantPubkeys.some((key) => !isHexPubkey(key))) {
    throw new ValidationError('participant_pubkeys must contain distinct valid pubkeys other than the creator');
  }
  ...
  if (escrow._idempotent) {
    ...
    const existingParticipantPubkeys = existingEnrollments
      .map((item) => item.participant_pubkey)
      .sort();
    const requestedParticipantPubkeys = [...participantPubkeys].sort();
    ...
    if (
      ...
      || JSON.stringify(existingParticipantPubkeys) !== JSON.stringify(requestedParticipantPubkeys)
    ) {
    ...
    enrollments = existingEnrollments
      ...
      .map((item) => ({ participant_pubkey: item.participant_pubkey, enrollment_token: item.enrollment_token }));
  } else {
    for (const participantPubkey of participantPubkeys) {
      const token = await createEnrollment(escrow.escrow_id, participantPubkey);
      enrollments.push({ participant_pubkey: participantPubkey, enrollment_token: token });
    }
  }
...
async function joinByEnrollment({ authPubkey, enrollmentToken, payoutLnAddress }) {
  const existing = await consumeEnrollment(enrollmentToken, authPubkey);
  const updated = await joinEscrowInstance(existing.escrow_id, authPubkey, payoutLnAddress);
  await redeemEnrollment(enrollmentToken, authPubkey);
  return {
...
async function cancel({ escrowId, authPubkey }) {
  ...
  if (escrow.funding_model !== 'single_funder') {
    for (const funder of (await listFunders(escrowId)).filter((row) => row.funded)) {
      const payout = await executePayout(escrow, funder.amount_sats, funder.refund_ln_address, funder.funder_pubkey);
